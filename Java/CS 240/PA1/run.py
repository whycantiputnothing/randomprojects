...
jdk_path = '/usr/lib/jvm/oracle-jdk-bin-1.7/bin/'

class Run():
    def __init__(self):
        self.path = 'java/'
        self.java_files = ['LinkedList.java',
                           'GenerateScore.java']
        self.start_class = 'GenerateScore'


    def compile_java(self, filename, classpath):
        cmd = jdk_path + 'javac -classpath ' + classpath + ' ' + classpath + filename
    ...
    def run_java(self, java_class, classpath, arg):
        cmd2 = jdk_path + 'java -classpath ' + classpath + ' ' + java_class + ' ' + arg
        print cmd2
