...
public class Gauss {
    ...
    private static double[][] gaussPartialPivot(double[][] matrix) {
        for (int k = 0; k < matrix[0].length; k++) { // for k = 1:n
            ...
            if ((k + 1) < matrix[0].length) { // zero indexed, so it checks if
                                              // there is another row
                max = findLargestPivot(matrix, k); // choose ip(k) such that
                                                   // |A(ip,k)|= max{|A(i,k)|: i
                                                   // >= k}
            }
            if (max == -1) { // if A(ipk,k) = 0
                ...
                return matrix;
            } else if (max != 0) { // wap A(k,k),..., A(k,n) with A(ipk,k),...,
                                   // A(ipk,n)
            ...
            for (int i = k + 1; i < matrix[0].length; i++) { // for i = k + 1:n
                ...
                matrix[i][k] = matrix[i][k] / matrix[k][k]; // A(i,k) =
                                                            // A(i,k)/A(k,k)

                if (debug)
                    System.out.println("A(i,K) after = " + matrix[i][k]);

                for (int j = k + 1; j < matrix[0].length; j++) { // for j = k +
                                                                 // 1:n
                    if (debug)
                        System.out.println("A(i,j) before = " + matrix[i][j]);

                    matrix[i][j] = matrix[i][j] - matrix[i][k] * matrix[k][j]; // A(i,j)
                                                                               // =
                                                                               // A(i,j)
                                                                               // -
                                                                               // A(i,k)
